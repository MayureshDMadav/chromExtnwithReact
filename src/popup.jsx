...
const Popup = () => {
  ...
  const background = () => {
    try {
        ...
        try {
          setTimeout(() => {
            ...
            let appTextInput = document
              ...
              .querySelectorAll("p");
            window.urlForTest = appTextInput[0].textContent;
            let appNameInput = "";
            if (appTextInput) {
              if (appTextInput[0].textContent.split(".")) {
                if (appTextInput[0].textContent.split(".").includes("www")) {
                  console.log("www is  available in the URL");
                  appNameInput = appTextInput[0].textContent.split(".")[1];
                } else {
                  console.log("www is not available in the URL");
                  appNameInput = appTextInput[0].textContent.split(".")[0];
                }
              }
            } else {
              appNameInput = window.location.href.split("/")[4];
            }
            ...
            checkBoxArr.forEach((element, index) => {
              if (
                [
                  ...
                  61, 63, 65, 67, 69, 71, 73, 75, 77, 79, 81, 82, 83, 84, 85,
                  87, 89, 91, 93, 94, 96, 98, 100, 102, 104, 106, 108,
                ].includes(index)
              )
                element.click();
            });
      ...
      const gettingStoreFrontApiScopesAndInstalling = (
        element,
        delay,
        words
      ) => {
        try {
          ...
          setTimeout(() => {
            console.log("Selecting StoreFront Api scopes");
          ...
          setTimeout(() => {
            ...
            let arrayOFButtons = Array.from(installApplicationButtonElement);
            if (
              arrayOFButtons[arrayOFButtons.length - 1].textContent ===
              "Install"
            ) {
        ...
        try {
          setTimeout(() => {
            ...
            let jsonResponse = {
              ...
              app_id: urlPatharr[6],
              merchant_url: window.urlForTest,
            };

            try {
              function encText(value) {
                encodedString = window.btoa(value);
                return encodedString;
              }

             
              let encData = encText(JSON.stringify(jsonResponse));

              if (jsonResponse) {
                const myHeaders = new Headers();
                myHeaders.append("Content-Type", "application/json");
                const raw = encData;
                const dataValidation = "addData";
                const url = `https://script.google.com/macros/s/AKfycbyrDbZ_hZ_uWliP-D9sqYrlO6L_On8C880eR5cUhsB5-LCpkcrazbDZtuidqulAEiXV/exec?action=${dataValidation}`;
                ...
                fetch(url, requestOptions)
                  ...
                  .then((result) => alert("App Successfully Installed !!"))
                  .catch((error) =>
                    alert("App Creation Failed please retry once again !!!")
                  );
              }
            } catch (e) {
              console.log(e);
      ...
      clickOnDelay("button[type='button']", 73000, "Reveal token once");
      fetChingCredentials("input[type]", 78000);
    } catch (e) {
      console.log(e);
